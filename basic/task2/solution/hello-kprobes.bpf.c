...
int BPF_KPROBE(probe_do_mkdirat_entry, int dfd, struct filename *f, umode_t mode)
{
	pid_t pid;
	const char *filename;
	...
	pid = bpf_get_current_pid_tgid() >> 32;
	filename = BPF_CORE_READ(f, name);
	bpf_printk("KPROBE: mkdirat -> pid = %d, dfd = %d, filename = %s, mode = %d\n", pid, dfd, filename, mode);
	return 0;
}
 ...
 */
SEC("kprobe/do_rmdir")
int BPF_KPROBE(probe_do_rmdir_entry, int dfd, struct filename *f)
{
	pid_t pid;
	const char *filename;
	pid = bpf_get_current_pid_tgid() >> 32;
	filename = BPF_CORE_READ(f, name);
	bpf_printk("KPROBE: rmdir -> pid = %d, filename = %s\n", pid, filename);
	return 0;
}
